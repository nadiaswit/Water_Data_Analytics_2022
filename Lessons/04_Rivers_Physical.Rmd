...
EnoDischarge <- readNWISdv(siteNumbers = "02085070",
                     ...
                     startDate = "", 
                     endDate = "2021-09-30") #USGS defines water year as Oct 1 - Sept 30; will capture winter season and not split it up
...
EnoDischarge <- EnoDischarge %>%
  mutate(Year = year(Date), 
         WaterYear = water_year(Date, origin = "usgs")) %>% #water year function based on date given; use usgs definition of water year
  filter(WaterYear != "1963") #remove 1963; R computes year as a factor, put in quotes

# Water Year is a factor. We want to re-classify as numeric.
EnoDischarge$WaterYear <- as.numeric(as.character(EnoDischarge$WaterYear)) #if factor, need to say character then numeric because it would instead rank instead of by year

EnoRecurrence <- 
  EnoDischarge %>%
  filter(WaterYear < 1994) %>% #can now call a number since R knows it is a number
  group_by(WaterYear) %>%
...
#recurrence interval - 31; probability is 3% that amount of more will occur 3% of the year
...
#over 91% of the variability in model can be explained by the reccurrence interval
```
...
#"nonremarkable years" happen more frequently, higher recurrence intervals with high discharge are not happening as frequently
...
#Eno river is not showing stationarity; discharge magnitude is increasing between recurrence intervals
...
#drop in peak discharge; built for flood control
```
