...
#North Temperate Lakes Ecological Research Station; Northern Wisconsin
#read_csv and read.csv - both can read csv files; however read_csv is faster and more efficient (used in dplyr package)
#here package works with relative file paths
...
class(NTLdata$sampledate) #$ references a column
NTLdata$sampledate <- as.Date(NTLdata$sampledate, format = "%m/%d/%y") #want to change column to date; % is how function deals with format to date/time information
...
dim(NTLdata)
NTLdata <- NTLdata %>% 
  drop_na(temperature_C) #don't need to specify column since already referencing dataset with function (dplyr) with the pipe function
#can also write in one line NTLdata <- drop_na(NTLdata, temperature_C) but easier to have a pipe to add more steps
#pipe always happens outside of a function
dim(NTLdata)  

# How many observations are there for each lake?
summary(NTLdata$lakename)
summary(as.factor(NTLdata$lakename)) #counts number of time lake name appears; removed NAs from temp also tells how many temp measures

# Let's choose the two lakes with the most data
NTLdata_PeterPaul <- NTLdata %>% 
  filter(lakename %in% c("Paul Lake", "Peter Lake")) #filter takes rows in dataframe; %in% = includes concatenation (Peter and Paul Lakes)
...
#epilimnion - upper water column (warmer)
#hypolimnion - lower water column (cooler)

#harder to mix water column when there is a big temperature difference; mix
#mictic = mixing
#due to thermal mass of water; cooler and warmer water have too much of a difference to mix; mixing due to warmer air temp; ice formation
#dimictic lake - mixes twice during year (spring and fall will have mixing)
#monomictic - mixes once; NC waters due to infrequent ice over
#polymictic - mixes several times per year; tend not to see much seasonal stratification; shallower lake due to less thermal mass in different layers
#amictic - always stratified, don't ever mix; very deep, sheltered lakes (bottom of hollow, experience a lot of weather patterns/storms)
...
DO_plot <- ggplot(subset(Pauldata, year4 == 1993), 
...
#DO still same in Nov; static
#DO concentrations in epilimnion (higher water column); wave motion incorporating oxygen into water (wave induced mixing); phytoplankton photsynthesizing during the day
#slightly higher DO above lower depths above thermocline; zooplankton hang out there; nutrients also coming up from bottom; respiration occuring in the hypolimnion (dead and decaying algae sinking down); phytoplankton hanging out where they're getting the most light but also the most nutrients without expending too much energy
#deep chlorophyll maximum

#limnic eruption
#higher concentrations of methane, CO2, NO2, as a product of anaerobic respiration
#Lake Niose - amictic but mixing only due to tectonic plates; release of gases resulted in destruction of town; Lake Hibu?
...
#metalimnion - has a band where there is some temperature change  
#thermocline deepening as the year goes on
```
...
ggplot(subset(Pauldata, depth == 1),
       aes(x = daynum, y = temperature_C, color = as.factor(year4))) + #can plot as continuous
  geom_point(alpha = 0.5) +
  geom_line() +
  scale_color_viridis_d()
# exercise: relabel the aesthetics

#expect to see interannual variability
