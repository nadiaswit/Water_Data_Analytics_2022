...
Impervious surfaces; topography of watershed and slope leading to time of concentration - larger that channels precipitation in larger discharge event; subsurface flow with soil; magnitude of stream; drainage area and number of tributaries (order of stream)
...
library(EcoHydRology) #overwrites use of pipe
library(tidyverse)
...
ggplot(EnoDischarge, aes(x = Date, y = Discharge)) +
  ...
  #if want to add new data set geom_line(data=something else, aes(y=baseflow)); can also assign plot and add geom_line(aes(y=))
  scale_y_log10() +
  labs(x = "Year", y = "Discharge (cfs)")
#different magnitudes - higher magnitude in storm flow compared to base flow
...
#Baseflow separation function calculates stormflow and baseflow

EnoDischarge <- cbind(EnoDischarge, EnoDischarge_basesep_EcoHydro) #able to combine because have the same number of rows
...
ggplot(EnoDischarge, aes(x = Baseflow_lfstat, y = bt)) +
  geom_point(alpha = 0.1, size = 1) + #alpha = 1 is not transparent; alpha = 0 is invisible (90% transparent)
  geom_abline(slope = 1, intercept = 0, color = "darkred") #1:1 line
#similar values at smaller values; different range of values for larger values; estimates diverge based on event based flows
#what? showing density of similar values - looking at how each model differs (darker color shows overlap)

# How would we test that statistically? [add code here]
#linear model - looking at slope of relationship between line
#paired t-test; but that compares means of group (looking at differences in coefficient can help distinguish)
...
EnoDischarge_summary <- EnoDischarge %>%
  group_by(WaterYear) %>%
  summarise(Discharge.acft.yr = sum(Discharge)*723.968, # what is this factor? converting to acre*ft/yr
 ...
 mutate_if(is.numeric, round, 2) #if numeric, round to 2 decimal places

EnoDischarge_summary$WaterYear <- as.numeric(as.character(EnoDischarge_summary$WaterYear))
class(EnoDischarge_summary$WaterYear)

ggplot(EnoDischarge_summary, aes(x = WaterYear, y = Discharge.acft.yr)) +
  geom_line() +
  geom_line(aes(y = Baseflow.acft.yr), lty = 2) #lty = changes line type
...
DodgeDischarge <- readNWISdv(siteNumbers = "07139500",
                     parameterCd = "00060", # discharge (ft3/s)
                     startDate="1960-10-01",
                     endDate = "2005-09-30")

names(DodgeDischarge)[4:5] <- c("Discharge", "Approval.Code")

attr(DodgeDischarge, "variableInfo")
attr(DodgeDischarge, "siteInfo")

DodgeDischarge_basesep_EcoHydro <- BaseflowSeparation(DodgeDischarge$Discharge)

DodgeDischarge <- cbind(DodgeDischarge, DodgeDischarge_basesep_EcoHydro)

ggplot(DodgeDischarge, aes(x = Date, y = Discharge)) +
...
Overlays 
Over aquifer
See non-perennial flows at Dodge City due to pumping
Gaining stream becomes losing (because losing to GW pumping)
