...
---
How to get color in legend
ggplot(cars, aes(x=speed))+
  geom_line(aes(y=dist, color = "total discharge"))+ #can define what aeesthetic is called
  scale_color_manual(values=c("ttotal discharge"="gray")) #can then define what color it will be; can use multiple colors if unique names; want to name several colors (naming values with c("gray", "blue", "teal")
  need to point to label we just created
  can also do with linetype, scale_linetype
  
if color scale - would need a continuous variable with color scale
...
options(scipen = 4) #sets scientific notation - how many numbers to report out instead of exponential notation

lagosne_get(dest_folder = LAGOSNE:::lagos_path(), overwrite = TRUE)
#download lagose database to machine
#different datasets within database
...
LAGOSlocus <- LAGOSdata$locus #using $ because it is an object within the LAGOS list
...
#these databased follow a star schema, so they can be linked together easily
```
...
#left join will typically find the same column if named the same; but if different then need to specify name
#by=c("colname1", "colname2")
...
#relevel, levels to reorder factor levels along x axis
...
**0-40**: Oligotrophic #low
**40-50**: Mesotrophic #medium
**50-70**: Eutrophic #high
**70-100**: Hypereutrophic #very high
...
ggplot(LAGOStrophic, aes(x = as.factor(samplemonth), y = chla)) +
  geom_jitter(alpha = 0.2, size = 0.5) + #putting geom jitter can show more information behind/in front of boxplot
...
#higher in winter months; indicate higher clarity; higher levels of chlorophyll in summer
...
#median is level; all the TP in water column (dissolved and within plankton); shifts in fractions are still accounted for in water column
```
...
LAGOStrophic <- LAGOStrophic %>%
  ...
  mutate(TSI.chl = round(10*(6 - (2.04 - 0.68*log(chla)/log(2)))),
         ...
         trophic.class = case_when(TSI.chl < 40 ~ "Oligotrophic", #similar to if else function
...
#map for HEX codes for continuous color palette

# Let's save the LAGOStrophic dataset as a csv in our Data folder. 

write.csv(LAGOStrophic, file = "./Data/Processed/LAGOStrophic.csv", row.names = FALSE) #removes row names that has numbers
```
